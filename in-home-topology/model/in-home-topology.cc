      ...
      for (uint16_t devIndex = 0; devIndex < m_dev.size (); devIndex++)
        {
          ...
          NS_LOG_DEBUG("Device index: " << devIndex << ", id: " << m_dev.at (devIndex).id << ", neighbour: " << m_dev.at (devIndex).neighbour
                  << ", x: " << m_dev.at (devIndex).c.x << ", y: " << m_dev.at (devIndex).c.y);
        }
      break;
    }
...
//          NS_ASSERT (devIndexG < m_devEnergy.size ());
  ...
  for (uint16_t devIndex = 0; devIndex < m_devs.size (); devIndex++)
    {
      NS_LOG_DEBUG(m_devs.at (devIndex).id << "\t" << m_devs.at (devIndex).neighbour << "\t" << m_devs.at (devIndex).c.x << "\t" << m_devs.at (devIndex).c.y);
    }
      ...
      for (uint16_t devIndex = 0; devIndex < m_devs.size (); devIndex++)
        {
          if (m_devs.at (devIndex).energy == 1)
            {
              NS_LOG_DEBUG("Setting position of modem dev " << m_devs.at (devIndex).id << ", which associates with node (index) " << nodeIndex);
      ...
      for (uint16_t devIndex = 0; devIndex < m_devs.size (); devIndex++)
        {
          if (m_devs.at (devIndex).energy != 0 && m_devs.at (devIndex).energy != 1)
            {
              NS_LOG_DEBUG("Setting position of electrical device  dev " << m_devs.at (devIndex).id << ", which associates with node (index) " << nodeIndex);
              nodes.at (nodeIndex)->SetPosition (m_devs.at (devIndex).c.x, m_devs.at (devIndex).c.y, 0);
              m_devs.at (devIndex).nodeIndex = nodeIndex;
              nodeIndex++;
            }
      ...
      for (uint16_t devIndex = 0; devIndex < m_devs.size (); devIndex++)
        {
          if (m_devs.at (devIndex).energy == 0)
            {
              NS_LOG_DEBUG("Setting position of distribution box  dev " << m_devs.at (devIndex).id << ", which associates with node (index) " << nodeIndex);
              nodes.at (nodeIndex)->SetPosition (m_devs.at (devIndex).c.x, m_devs.at (devIndex).c.y, 0);
              m_devs.at (devIndex).nodeIndex = nodeIndex;
              nodeIndex++;
            }
      ...
      for (uint16_t devIndex2 = 0; devIndex2 < m_devs.size (); devIndex2++)
        {
          ...
          if (m_devs.at (devIndex).neighbour == m_devs.at (devIndex2).id)
            {
              NS_LOG_DEBUG("Connecting devs with ids " << m_devs.at (devIndex).id << " and " << m_devs.at (devIndex2).id
                      << " associated with nodes (indexes) " << m_devs.at (devIndex).nodeIndex << " and " << m_devs.at (devIndex2).nodeIndex);
              CreateObject<PLC_Line> (cable, nodes.at (m_devs.at (devIndex).nodeIndex), nodes.at (
                      m_devs.at (devIndex2).nodeIndex));
              //
